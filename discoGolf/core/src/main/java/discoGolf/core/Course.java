...
public class Course {
  ...
  /**
   * Create a disc golf course by validating courseName and parValues, also initialize courseHoles
   * by creating Hole object with belonging par value.
   *
   * @param courseName - is the name of the course.
   * @param parValues - is an array of integers that represents the par value for the hole with
   *        number == to its array index.
   */
  public Course(String courseName, ArrayList<Integer> parValues) {
    ...
    this.courseHoles =
        parValues.stream().map(p -> new Hole(p)).collect(Collectors.toCollection(ArrayList::new));
  ...
  /**
   * course name contains the name of the course.
   *
  ...
  /**
   * the course holes in an arraylist containing Hole objects.
   *
   * @return ArrayList named courseHoles containing all the Hole objects in the course.
   */
  ...
  /**
   * the total number of holes for a scorecard.
   *
  ...
  /**
   * iterate through the courseHoles list and collect the par value for each hole.
   *
   * @return ArrayList named parValues - containing the par values for each hole.
   */
  public ArrayList<Integer> getParValues() {
    return courseHoles.stream().map(p -> p.getPar())
   ...
   * @throws IllegalArgumentException if the list is empty or contains values outside the range 2-7
   *         and not empty.
   */
