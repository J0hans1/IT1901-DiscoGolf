...
/**
 * Leadboard class that holds alle the saved Scorecard objects in a hashmap connected to their.
 * course, in sorted order.
 *
...
public class Leaderboard {
   ...
   *
   * @param dataObject Data object that contains saved Scorecards.
   */
  ...
  /**
   * Create a HashMap where each key is a unique courseName saved in the database. Each key maps to
   * an arrayList that holds each Scorecard connected to that unique course.
   *
  ...
  /**
   * the leaderboard hashMap containing alle the different leaderboards.
   *
   * @return leaderboardForAllCourses hashamap containing courseNames as keys, and realated
   *       arraylists with scorecard objects as values.
   */
   ...
   */
  private ArrayList<ScorecardInterface> sortLeaderboardList(
      ArrayList<ScorecardInterface> scorecardLeaderboard) {
    Collections.sort(scorecardLeaderboard, new LeaderboardComparator());
    return scorecardLeaderboard;
  }

  /**
   * Add scorecard to relevant list in leaderboardForAllCourses based on which course the scorecard
   * object contains. Also sort the relevant list.
   *
