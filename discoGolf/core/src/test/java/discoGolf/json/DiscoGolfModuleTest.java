...
public class DiscoGolfModuleTest {
  ...
  /**
   * Test serializers by comparing the dataInJsonFormat with the data object as string using
  ...
  /**
   * Test deserializers by reading the string dataInJsonFormat and compare the scorecard object to
   * expected values
   */
  @Test
  public void testDeserializers() {
    try {
      ...
      assertEquals(new ArrayList<>(Arrays.asList(3, 4, 3, 4, 3, 4, 3, 4, 3)),
          scorecardRead.getCourse().getParValues());
    } catch (JsonProcessingException e) {
      fail(e.getMessage());
    }
  }
  ...
  /**
   * Test both serializers and deserializers by writing the createDataObject as string, then read
   * the string with mapper.readValue and comparing the new scorecard pbject in the data object to
   * expected values
   */
  @Test
